...
import { isONNXTensor, Tensor as ONNXTensor } from "../backends/onnx.js";
import { TensorOpRegistry } from "../ops/registry.js";
import { interpolate_data, max, min, permute_data } from "./maths.js";

const tensorRegistry = new FinalizationRegistry((ort) => {
  try {
    ort.dispose();
  } catch (e) {
    // Tensor already disposed or context lost
  }
});
...
export class Tensor {
  ...
  static __deserialize__(data) {
    return new Tensor(data.type, data.data, data.dims);
  }
  ...
  *[Symbol.iterator]() {
  ...
  quantile(q, dim = null, keepdim = false) {
    ...
    if (Number.isInteger(index)) {
      return new Tensor(this.type, [data[index]], []);
    }
 ...
 *
 * @param {Tensor|ONNXTensor} tensor - The input tensor.
 * @param {string} dtype - Target data type (e.g., "float32", "float16").
 * @returns {Tensor|ONNXTensor} - Converted tensor.
 * @throws {Error} - If the tensor type is unsupported.
 */
export function to(tensor, dtype) {
...
export async function interpolate_4d(input, { size = null, mode = "bilinear" } = {}) {
  ...
  const sizeTensor = new Tensor("int64", new BigInt64Array(targetDims.map(BigInt)), [
    targetDims.length,
  ]);
...
export function std_mean(input, dim = null, correction = 1, keepdim = false) {
  ...
  if (dim === null) {
    ...
    const std = Math.sqrt(
      inputData.reduce((a, b) => a + (b - mean) ** 2, 0) / (inputData.length - correction)
    );
...
/**
 * Raises each element of the input tensor to the specified exponent.
 * @param {Tensor} tensor The input tensor.
 * @param {number} exponent The exponent to raise each element to.
 * @returns {Tensor} A new tensor with each element raised to the given exponent.
 */
export function pow(tensor, exponent) {
